...
<script>
import { getPersonalDetails, getJobDetail } from "@/api/employees";
...
export default {
  ...
  // 创建完毕状态
  created() {
    this.type === "personal" ? this.getPersonalDetails() : this.getJobDetail();
  },
  // 组件更新
  methods: {
    async getPersonalDetails() {
      this.formData = await getPersonalDetails(this.userId); // 获取个人基本信息
    },
    async getJobDetail() {
      const userInfo = await getUserDetailsById(this.userId);
